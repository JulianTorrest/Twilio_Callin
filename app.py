                        ...
                        if not llamada_activa:
                            ...
                            with col_btn1:
                                if st.button("💾 Guardar Notas", key=f"save_notes_{idx}", width='stretch'):
                                    # Obtener nota existente del draft
                                    nota_existente = str(st.session_state.draft_notas.get(idx, ''))
                                    nota_actual = st.session_state.draft_notas.get(idx, '')
                                    
                                    # Combinar notas si ya existe algo
                                    if nota_existente and nota_existente.strip():
                                        if nota_actual and nota_actual != nota_existente:
                                            ...
                                            nota_acumulada = f"{nota_existente} | [{timestamp}] ({categoria_actual}) {nota_actual}"
                                        else:
                                        ...
                                        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
                                        categoria_actual = st.session_state.df_contactos.at[idx, 'estado'] if 'estado' in st.session_state.df_contactos.columns else 'Sin categoría'
                                        nota_acumulada = f"[{timestamp}] ({categoria_actual}) {nota_actual}"  # Primera nota con timestamp y categoría
                                    ...
                                    # 🔥 NO HACER RERUN INMEDIATO para evitar que desaparezca el usuario
                                    # st.rerun()  # Comentado para evitar desaparición
                            
                            with col_btn2:
                                # 🔥 USAR SESSION_STATE PARA CONTROLAR DIÁLOGO DE REPROGRAMACIÓN
                                reprogramar_dialogo_key = f"reprogramar_dialogo_{idx}"
                                ...
                                if st.session_state.get(reprogramar_dialogo_key, False):
                                    ...
                                    col_prog_cancel, col_prog_confirm = st.columns(2)
